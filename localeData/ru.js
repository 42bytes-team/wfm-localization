!function (e, a) {
    ...
    return [{
        ...
        pluralRuleFunction: function (e, a) {
            let t = String(e).split('.'), r = t[0], o = !t[1], n = r.slice(-1), l = r.slice(-2);
            return a ? 'other' : o && 1 == n && 11 != l ? 'one' : o && n >= 2 && n <= 4 && (l < 12 || l > 14) ? 'few' : o && 0 == n || o && n >= 5 && n <= 9 || o && l >= 11 && l <= 14 ? 'many' : 'other';
        },
        fields: {
            year: {
                displayName: 'год',
                relative: {0: 'в этом году', 1: 'в следующем году', '-1': 'в прошлом году'},
                relativeTime: {
                    future: {
                        one: 'через {0} год',
                        few: 'через {0} года',
            ...
            day: {
                displayName: 'день',
                relative: {0: 'сегодня', 1: 'завтра', 2: 'послезавтра', '-2': 'позавчера', '-1': 'вчера'},
                relativeTime: {
                    future: {
                        one: 'через {0} день',
                        few: 'через {0} дня',
    ...
    }, {locale: 'ru-BY', parentLocale: 'ru'}, {locale: 'ru-KG', parentLocale: 'ru'}, {
        locale: 'ru-KZ',
        parentLocale: 'ru'
    }, {locale: 'ru-MD', parentLocale: 'ru'}, {locale: 'ru-UA', parentLocale: 'ru'}];
});
